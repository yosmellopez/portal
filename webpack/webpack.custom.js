...
module.exports = async (config, options, targetOptions) => {
  const languagesHash = await hashElement(path.resolve(__dirname, '../frontend/i18n'), {
    algo: 'md5',
    encoding: 'hex',
    files: { include: ['*.json'] },
  });
  ...
  const patterns = [
    ...
    { from: './frontend/swagger-ui/', to: 'swagger-ui/' },
  ...
  config.plugins.push(
    ...
    new MergeJsonWebpackPlugin({
      output: {
        groupBy: [
          { pattern: './frontend/i18n/es/*.json', fileName: './i18n/es.json' },
          { pattern: './frontend/i18n/en/*.json', fileName: './i18n/en.json' },
